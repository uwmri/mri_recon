...
Usage Example:
        ...
        This is a speciallized gridding routine especially for simultaneous
multislice with sliding slices
...
void smsEncode::alloc_grid() {
  ...
  cout << "Allocating Gridding Array: " << Sx << " x " << Sy << " x " << Sz
       << " :: " << (Nt + sms_factor - 1) << " x " << Ne << endl;
  Array<Array<complex<float>, 3>, 2> temp =
      Alloc5DContainer<complex<float> >(Sx, Sy, Sz, Nt + sms_factor - 1, Ne);
...
void smsEncode::precalc_gridding(int NzT, int NyT, int NxT, int NtT, int NeT,
                                 float sms_factorT, MRI_DATA &data) {
  (void)sms_factorT;
  Nx = NxT;
  Ny = NyT;
  Nz = NzT;
 ...
 */
void smsEncode::forward(
    Array<Array<complex<float>, 3>, 2> &xdf, Array<complex<float>, 3> &smap,
    Array<Array<complex<float>, 3>, 2> &data, Array<Array<float, 3>, 2> &kx,
    Array<Array<float, 3>, 2> &ky, Array<Array<float, 3>, 2> &kz,
    Array<Array<float, 3>, 2> &kw, Array<Array<float, 3>, 3> &z) {
  tictoc T;
  if (time_grid) T.tic();
 ...
 * @param kw k-space weight
 */
void smsEncode::backward(
    Array<Array<complex<float>, 3>, 2> &X, Array<complex<float>, 3> &smap,
    Array<Array<complex<float>, 3>, 2> &data, Array<Array<float, 3>, 2> &kx,
    Array<Array<float, 3>, 2> &ky, Array<Array<float, 3>, 2> &kz,
    Array<Array<float, 3>, 2> &kw, Array<Array<float, 3>, 3> &z) {
  tictoc T;
...
void smsEncode::accumulate(Array<Array<complex<float>, 3>, 2> &X,
      ...
      for (int k = 0; k < Nz; k++) {
        for (int j = 0; j < Ny; j++) {
          ...
          for (int i = 0; i < Nx; i++) {
            ...
            X(t, e)
            (i, j, k) += wt * k3d_grid(t, e)(i + og_sx, j + og_sy, k) *
                         conj(smap(i, j, k));
          }
        }
      }
...
void smsEncode::set_image(Array<Array<complex<float>, 3>, 2> &X,
                          Array<complex<float>, 3> &smap) {
  for (int t = 0; t < X.length(firstDim); t++) {
    for (int e = 0; e < X.length(secondDim); e++) {
#pragma omp parallel for
      for (int k = 0; k < Nz; k++) {
        for (int j = 0; j < Ny; j++) {
          ...
          for (int i = 0; i < Nx; i++) {
            float wt = wty * winx(i + og_sx);
            k3d_grid(t, e)(i + og_sx, j + og_sy, k) =
                wt * X(t, e)(i, j, k) * smap(i, j, k);
          }
        }
      }
    }
        ...
        for (int ii = 0; ii < data(t, e).length(firstDim); ii++) {
          ...
          for (int lz = sz; lz <= ez; lz++) {
            ...
            for (int ly = sy; ly <= ey; ly++) {
              ...
              complex<float> wty =
                  Cwtz * ((float)(grid_filterY((int)dely) * (1.0 - dy) +
              ...
              for (int lx = sx; lx <= ex; lx++) {
                ...
                complex<float> wtx =
                    wty * ((float)(grid_filterX((int)delx) * (1.0 - dx) +
                ...
                float RD = real(temp2);
                float ID = imag(temp2);
                float *I = reinterpret_cast<float *>(
                    &k3d_grid(t + sms_pos, e)(lx, ly, lz));
                float *R = I++;
