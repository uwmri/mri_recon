        ...
        for (int k = 0; k < Xref.length(thirdDim); k++) {
          for (int j = 0; j < Xref.length(secondDim); j++) {
            for (int i = 0; i < Xref.length(firstDim); i++) {
              complex<float> val =
                  complex<float>(1.0 / 6.0, 0.0) *
                  (complex<float>(6.0, 0.0) * Xref(i, j, k) -
                   Xref((i + 1) % Nx, j, k) - Xref((i + Nx - 1) % Nx, j, k) -
                   Xref(i, (j + 1) % Ny, k) - Xref(i, (j + Ny - 1) % Ny, k) -
                   Xref(i, j, (k + 1) % Nz) - Xref(i, j, (k + Nz - 1) % Nz));
      ...
      for (int k = 0; k < X.length(thirdDim); k++) {
        for (int j = 0; j < X.length(secondDim); j++) {
          for (int i = 0; i < X.length(firstDim); i++) {
            complex<float> val =
                complex<float>(1.0 / 6.0, 0.0) *
                (complex<float>(6.0, 0.0) * X(i, j, k) - X((i + 1) % Nx, j, k) -
                 X((i + Nx - 1) % Nx, j, k) - X(i, (j + 1) % Ny, k) -
                 X(i, (j + Ny - 1) % Ny, k) - X(i, j, (k + 1) % Nz) -
                 X(i, j, (k + Nz - 1) % Nz));
            transform_energy += norm(val);
          }
        }
...
// Regularize Function
//-----------------------------------------------------
void L2REG::regularize(Array<complex<float>, 3> &Rref,
                       Array<complex<float>, 3> &Xref) {
  if (lambda == 0.0) {
    return;
  }
...
#pragma omp parallel for
      for (int k = 0; k < Xref.length(thirdDim); k++) {
        for (int j = 0; j < Xref.length(secondDim); j++) {
          for (int i = 0; i < Xref.length(firstDim); i++) {
            Rref(i, j, k) +=
                complex<float>(reg_scale / 6.0, 0.0) *
                (complex<float>(6.0, 0.0) * Xref(i, j, k) -
                 Xref((i + 1) % Nx, j, k) - Xref((i + Nx - 1) % Nx, j, k) -
                 Xref(i, (j + 1) % Ny, k) - Xref(i, (j + Ny - 1) % Ny, k) -
                 Xref(i, j, (k + 1) % Nz) - Xref(i, j, (k + Nz - 1) % Nz));
...
#pragma omp parallel for
      for (int k = 0; k < Xref.length(thirdDim); k++) {
        for (int j = 0; j < Xref.length(secondDim); j++) {
          for (int i = 0; i < Xref.length(firstDim); i++) {
            Rref(i, j, k) +=
                reg_scale * complex<float>(0.0, imag(Xref(i, j, k)));
          }
        }
      }
    } break;
    ...
    case (LOWRES): {
      ...
      // Zeropadded bluring
      if ((int)ZeroPad.numElements() !=
          ((rcxres + 64) * (rcyres + 64) * (rczres + 64))) {
      ...
      for (int k = 0; k < ZeroPad.length(thirdDim); k++) {
        for (int j = 0; j < ZeroPad.length(secondDim); j++) {
          for (int i = 0; i < ZeroPad.length(firstDim); i++) {
            float rad = sqrt(((float)k - cz) * ((float)k - cz) +
                             ((float)j - cy) * ((float)j - cy) +
                             ((float)i - cx) * ((float)i - cx));
            rad /= 64.0;
            if (rad < 1) {
              ZeroPad(i, j, k) *=
                  (0.355768 - 0.487396 * cos(3.14159265359 * rad) +
                   0.144232 * cos(2 * 3.14159265359 * rad) -
                   0.012604 * cos(3 * 3.14159265359 * rad));
            }
          }
        }
      }
