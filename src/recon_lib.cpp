...
Array< Array<complex<float>,3 >,2 > RECON::test_sms( MRI_DATA& data_cal, MRI_DATA& data, int argc, char **argv){
    ...
    cout << "Sms Matrix = [ " << rcxres << "," << rcyres << "," << rczres << "]" << endl;
...

    {
        ...
        Array< Complex3D,2>TEMP5 = Alloc5DContainer< complex<float> >(rcxres,rcyres,rczres,rcframes,sms_encodes);
			...
			for(int e=0; e< rcencodes; e++){
				for(int t=0; t< Nt; t++){
                    ...
                    if( this->parallel_coils){
                        ...
                        if(recon_type==PILS){
                            gridding_CoilThreaded.forward(X(store_t,e),smaps,TEMP,data.kx(act_e),data.ky(act_e),data.kz(act_e),TimeWeight);
                        }else{
                            gridding_CoilThreaded.forward_sos(X(store_t,e),TEMP,data.kx(act_e),data.ky(act_e),data.kz(act_e),TimeWeight);
                        }
                    }else{
