    ...
    for (int z = -krz; z <= krz; z++) {
      for (int y = -kry; y <= kry; y++) {
        for (int x = -krx; x <= krx; x++) {
          float rad;
          if (krz_f == 0) {
            rad = (float)(x * x) / (krx_f * krx_f) +
                  (float)(y * y) / (kry_f * kry_f);
          } else {
            rad = (float)(x * x) / (krx_f * krx_f) +
                  (float)(y * y) / (kry_f * kry_f) +
                  (float)(z * z) / (krz_f * krz_f);
          }
          if (rad <= 1) {
            num_kernel++;
          }
      ...
      for (int iy = -cry; iy <= cry; iy++) {
        for (int ix = -crx; ix <= crx; ix++) {
          ...
          if (shape == SP_SQUARE) {
            ...
            rad =
                (float)(ix * ix) / (crx * crx) + (float)(iy * iy) / (cry * cry);
          } else {
            rad = (float)(ix * ix) / (crx * crx) +
                  (float)(iy * iy) / (cry * cry) +
                  (float)(iz * iz) / (crz * crz);
          }
          if (rad > 1) continue;
              ...
              for (int jy = -kry; jy <= kry; jy++) {
                for (int jx = -krx; jx <= krx; jx++) {
                  ...
                  if (shape == SP_SQUARE) {
                    ...
                    krad = (float)(jx * jx) / (krx_f * krx_f) +
                           (float)(jy * jy) / (kry_f * kry_f);
                  } else {
                    krad = (float)(jx * jx) / (krx_f * krx_f) +
                           (float)(jy * jy) / (kry_f * kry_f) +
                           (float)(jz * jz) / (krz_f * krz_f);
                  }
                  ...
                  if (krad <= 1) {
                    ...
                    A(Arow, Acol) = (complex<float>)kdata(ie, jc)(
                        kern_xx, kern_yy, kern_zz);
                    Acol++;
...
void SPIRIT::prep() {
  ...
  cout << "Hybrid Kernel Size = " << sx << " x " << sy << " x " << sz << " x "
       << nc << " x " << nV << endl;
...
void SPIRIT::getcoils(Array<Array<complex<float>, 3>, 1> &LR) {
  ...
  for (int iz = 0; iz < Nz; iz++) {
    ...
    for (int iy = 0; iy < Ny; iy++) {
      ...
      Array<complex<float>, 3> x_line =
          temp(all, iy, all, all);  // fixed y and z
...
#pragma omp parallel for
      for (int ix = 0; ix < Nx; ix++) {
        ...
        // SVD_econ is much faster than eig_sym and orthogonal iteration (but
        // still slow).
        cx_mat U;
        vec s;
        cx_mat V;
 ...
 * The rotation is weighted by the norm of the data's magnitude.  This still
 * ends up leaving some small discontinuities, but not many.
 */
