 ...
 * Code to setup a wavelet transform based on 3D Array
 * @see setup()
 * This code check to ensure the number of levels is not greater than that
 * allowed by the wavelet basis.
 */
...
void WAVELET3D::forward(Array<complex<float>, 5> &Coef) {
  for (int e = 0; e < Coef.extent(fifthDim); e++) {
    for (int t = 0; t < Coef.extent(fourthDim); t++) {
      Array<complex<float>, 3> CoefRef =
          Coef(Range::all(), Range::all(), Range::all(), t, e);
      forward3D(CoefRef);
    }
  }
}
...
void WAVELET3D::forward(Array<complex<float>, 4> &Coef) {
  for (int t = 0; t < Coef.extent(fourthDim); t++) {
    Array<complex<float>, 3> CoefRef =
        Coef(Range::all(), Range::all(), Range::all(), t);
    forward3D(CoefRef);
  }
}
 ...
 * @param Coef Array to be transformed (in-place)
 */
void WAVELET3D::forward(Array<complex<float>, 3> &Coef) { forward3D(Coef); }

/**
 * Call for backward transform
 * @param Coef Array to be transformed (in-place)
 */
void WAVELET3D::backward(Array<complex<float>, 3> &Coef) { backward3D(Coef); }
 ...
 * @param level
 */
void WAVELET3D::get_subband_range(NDarray::Range &rx, NDarray::Range &ry,
                                  NDarray::Range &rz, int lx, int ly, int lz) {
  int sz = 0;
  int sy = 0;
  int sx = 0;
  int ex = N[0];
 ...
 */
void WAVELET3D::wave_x(Array<complex<float>, 3> &Coef, int Lz, int Ly, int Lx,
                       int dir) {
  // cout << "Wave X = " << Le << "," << Lt << "," << Lz << "," << Ly << "," <<
  // Lx << "  Dir = " << dir << endl;
 ...
 * @param Lz  Size of subarray to be transformed
 */
void WAVELET3D::wave_y(Array<complex<float>, 3> &Coef, int Lz, int Ly, int Lx,
                       int dir) {
  // cout << "Wave Y = " << Le << "," << Lt << "," << Lz << "," << Ly << "," <<
  // Lx << "  Dir = " << dir << endl;
 ...
 * @param Lz  Size of subarray to be transformed
 */
void WAVELET3D::wave_z(Array<complex<float>, 3> &Coef, int Lz, int Ly, int Lx,
                       int dir) {
  // cout << "Wave Z = " << Le << "," << Lt << "," << Lz << "," << Ly << "," <<
  // Lx << "  Dir = " << dir << endl;
 ...
 */
void WAVELET3D::wave1D(complex<float> in[], complex<float> out[], int pts,
                       int dir) {
  /*Scaling Coef*/
 ...
 * @param dir  direction(x,y,z) for shifts
 */
void WAVELET3D::iwave1D(complex<float> in[], complex<float> out[], int pts,
                        int dir) {
  ...
  for (int pos = 0; pos < pts / 2; pos++) {
    ...
    for (int fp = 0; fp < wN; fp++) {
      int cpos = ((2 * pos + fp - wN + 1 + S[dir]) + pts) %
                 pts;  // Three is for shift correction
