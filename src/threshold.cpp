...
#define char_trig_flag(num, name, name2, val) \
  }                                           \
  else if (strcmp(name, pstring[pos]) == 0) { \
    pos++;                                    \
    if (strcmp(name2, pstring[pos]) == 0) {   \
      val = num;                              \
      pos++;                                  \
    }
  ...
  for (int pos = 0; pos < numarg; pos++) {
    if (strcmp("-h", pstring[pos]) == 0) {
      ...
      pos++;
      if (pos == numarg) {
        cout << "Please provide threshold type..fraction/bayes/visu/sure/fixed"
             << endl;
      ...
      pos++;
      if (pos == numarg) {
        cout << "Please specify setting for grouped complex "
                "thresholding..on/off (default is on)"
             << endl;
      ...
      pos++;
      if (pos == numarg) {
        cout << "Please provide noise estimation type..global/frame/first/last"
             << endl;
...
void THRESHOLD::help_message() {
  ...
  cout << "----------------------------------------------" << endl;
  help_flag("-threshold_type []",
            "thresholding method fixed/fraction/visu/bayes/sure");
  help_flag("-thresh []",
            "fraction of coefficient to be removed for fractional method "
            "(default=0.0)");
  help_flag("-noise_est_type []",
            "method for noise estimation global(estimate using all "
            "frames)/frame(estimate for each frame)/last(estimate using last "
            "frame)/first(estimate using first frame) (default=frame)");
  help_flag(
      "-noise_scale []",
      "scale factor for noise estimate(s) (e.g. for zero-padding correction)");
  help_flag("-thmode []", "soft/hard (default=soft)");
  help_flag("-th_group_complex []",
            "on/off --> on performs group soft thresholding of complex data. "
            "When off (default) re/im data is treated individually");
 ...
 *------------------------------------------------------------------*/
void THRESHOLD::update_threshold(Array<Array<complex<float>, 3>, 2> &Coef,
                                 WAVELET3D &wave, float scale) {
  switch (threshold_type) {
    case TH_FIXED: {
      ...
      if (independent) {
        ...
        for (int e = 0; e < Coef.length(secondDim); e++) {
          for (int t = 0; t < Coef.length(firstDim); t++) {
            ...
            if (VERBOSE) {
              cout << "Thresh (" << t << "," << e
                   << ") = " << independent_thresholds(t, e) << endl
                   << flush;
            }
          }
        }
 ...
 *------------------------------------------------------------------*/
float THRESHOLD::get_threshold(Array<Array<complex<float>, 3>, 2> &Coef,
                               float fraction) {
  float value;
...
/*---------------------------------------------------------------------------------------
  Calc threshold based on universal threshold method using noise variance
 (VisuShrink)
 *-------------------------------------------------------------------------------------*/
...
void THRESHOLD::get_visuthreshold(Array<Array<complex<float>, 3>, 2> &Coef,
  ...
  // VisuShrink tends to overestimate the threshold especially for large number
  // of samples Reduce it with an empirical factor The original formula is:
  // sigma_noise * sqrt(2*N_samples) May need a better scaling factor
  global_threshold =
      1.0 / 8.0 * noise *
      sqrt(2 * log(Coef(0, 0).numElements() * Coef.numElements() / 8.0));
}
...
void THRESHOLD::get_bayesthreshold(Array<Array<complex<float>, 3>, 2> &Coef,
  ...
  subband_threshold.resize(wave.L[0] + 1, wave.L[1] + 1, wave.L[2] + 1,
                           Coef.length(firstDim), Coef.length(secondDim));
  ...
  // Get Noise
  Array<Array<complex<float>, 3>, 2> Cf = Alloc5DContainer<complex<float> >(
      Coef(0, 0).length(firstDim), Coef(0, 0).length(secondDim),
      Coef(0, 0).length(thirdDim), 1, 1);
  ...
  for (int e = 0; e < Ne; e++) {
    for (int t = 0; t < Nt; t++) {
      if (noise_est_type == NOISE_FRAME) {
        ...
        robust_noise_estimate(Cf, wave);
        if (VERBOSE)
          cout << "Estimated noise (" << t << "," << e << "): " << noise
               << endl;
      }
      ...
      for (int lz = 0; lz < wave.L[2] + 1; lz++) {
        for (int ly = 0; ly < wave.L[1] + 1; ly++) {
          for (int lx = 0; lx < wave.L[0] + 1; lx++) {
            ...
            subband_threshold(lx, ly, lz, t, e) =
                get_bayesthreshold_subband(Subband);
            // cout << "Level(" <<lx<<","<<ly<<","<<lz<<") - thresh = " <<
            // subband_threshold(lx,ly,lz,0,0) << endl;
          }
        }
      }
    }
      ...
      for (int lz = 0; lz < wave.L[2] + 1; lz++) {
        for (int ly = 0; ly < wave.L[1] + 1; ly++) {
          for (int lx = 0; lx < wave.L[0] + 1; lx++) {
            ...
            // Get and store subband threshold	(
            subband_threshold(lx, ly, lz, t, e) =
                get_surethreshold_subband(Subband);
            // cout << "Level(" <<lx<<","<<ly<<","<<lz<<") - thresh = " <<
            // subband_threshold(lx,ly,lz,0,0) << endl;
          }
        }
...
void THRESHOLD::exec_multibandthreshold(
    Array<Array<complex<float>, 3>, 2> &Coef, WAVELET3D &wave) {
  int Ne = Coef.length(secondDim);
  int Nt = Coef.length(firstDim);
...
float THRESHOLD::sure_cost(Array<complex<float>, 3> &Coef, float thresh) {
  float count = 0.0;
  float energy = 0.0;
  for (Array<complex<float>, 3>::iterator miter = Coef.begin();
       miter != Coef.end(); miter++) {
    count += (float)(abs(*miter) < thresh);
    energy += pow(min(abs(*miter), thresh), 2.0f);
  }

  float N = Coef.numElements();
  // cout << "Noise = " << noise << ", N = " << N << ", Energy " << energy << "
  // ,count=" << count << endl;
...
float THRESHOLD::get_surethreshold_subband(Array<complex<float>, 3> &Coef) {
  ...
  for (int pos = 0; pos < sureres; pos++) {
    float thresh = 1.0 / 8.0 * noise *
                   sqrt(2 * log((float)Coef.numElements())) * ((float)pos) /
                   (((float)sureres) - 1.0);
...
void THRESHOLD::robust_noise_estimate(Array<Array<complex<float>, 3>, 2> &Coef,
                                      WAVELET3D &wave) {
  ...
  // The (now commented) code block below may cause problems with
  // multi-frame/encode data, especially when the number of frames (amount of
  // sampled data) varies significantly between frames/encodes (i.e. some are
  // noisier than others).
  /* *******************************************************************************************
          // Get 3D volume
          Array<complex<float>,3> HHHref;
          HHHref.reference(
  Coef(Coef.length(firstDim)-1,Coef.length(secondDim)-1)(rx,ry,rz));
  ...
  **********************************************************************************************/
  ...
  // Compute "global" noise from all provided frames and encodes (may be all or
  // some subset)
  Array<complex<float>, 3> HHHref = Coef(0, 0)(rx, ry, rz);
  Array<Array<complex<float>, 3>, 2> DH = Alloc5DContainer<complex<float> >(
      HHHref.length(firstDim), HHHref.length(secondDim),
      HHHref.length(thirdDim), Nt, Ne);
  ...
  cout << "Size of array for noise estimation -- (nt,ne): " << DH.shape()
       << " --- (nx,ny,nz): " << DH(0, 0).shape() << endl;

  // Noise Estimate = median/ 0.6745	 - noise scale is needed for zero filled
  // data
  noise = get_threshold(DH, 0.5) / 0.6745 * noise_scale;
}

// Option of thresh of appriximation wavelets or all wavelets, hard vs soft
// threshold
void THRESHOLD::thresholding(Array<Array<complex<float>, 3>, 2> &Coef,
  ...
  if (!group_complex) {
    cout << " Using independent Threshold. Imaginary scale =" << imaginary_scale
         << endl;
  }
...
void THRESHOLD::setThresholdMethod(int type) { threshold_type = type; }

int THRESHOLD::getThresholdMethod() { return threshold_type; }

void THRESHOLD::setTemporalThresholding(bool flag) { temporal = flag; }

bool THRESHOLD::getTemporalThresholding() { return temporal; }
