...
/**
 * Initialization of phantom. This function does all the setup (build fractal
 * tree, read external, etc) before looped calls.
...
void PHANTOM::init(int Sx, int Sy, int Sz, int St) {
  ...
  switch (phantom_type) {
    ...
    case (EXTERNAL): {
      cout << "External Phantom Read " << external_phantom_name
           << " on the fly " << endl;
      // Read on the fly
    } break;
  }
...
/**
 * Get a sensitivity based on simulated coil geometry with Biot-Savart
 * simulation of field.
...
void PHANTOM::update_smap_biotsavart(int coil, int Ncoils) {
  ...
  Elements are placed in the Surface of a cylinder with beveled edges for
  overlap.
  */
  ...
  for (int pos = 0; pos < RES; pos++) {
    ...
    loop_pos(2, count) =
        (coil_length - overlap_length) * ((float)pos / (float)RES);
    count++;
  }
  ...
  for (int pos = 0; pos < RES; pos++) {
    loop_pos(0, count) =
        coil_radius *
        cos(-overlap_angle + (2 * overlap_angle) * ((float)pos / (float)RES));
    loop_pos(1, count) =
        coil_radius *
        sin(-overlap_angle + (2 * overlap_angle) * ((float)pos / (float)RES));
    loop_pos(2, count) = (coil_length - overlap_length) +
                         overlap_length * ((float)pos / (float)RES);
    count++;
  }
  ...
  for (int pos = 0; pos < RES; pos++) {
    loop_pos(0, count) =
        coil_radius *
        cos(overlap_angle + (2 * PI / (float)Ncoils - 2.0 * overlap_angle) *
                                ((float)pos / (float)RES));
    loop_pos(1, count) =
        coil_radius *
        sin(overlap_angle + (2 * PI / (float)Ncoils - 2.0 * overlap_angle) *
                                ((float)pos / (float)RES));
  ...
  for (int pos = 0; pos < RES; pos++) {
    loop_pos(0, count) =
        coil_radius * cos(2 * PI / (float)Ncoils - overlap_angle +
                          (2 * overlap_angle) * ((float)pos / (float)RES));
    loop_pos(1, count) =
        coil_radius * sin(2 * PI / (float)Ncoils - overlap_angle +
                          (2 * overlap_angle) * ((float)pos / (float)RES));
    loop_pos(2, count) =
        coil_length - overlap_length * ((float)(pos + 1) / (float)RES);
    count++;
  }
  ...
  for (int pos = 0; pos < RES; pos++) {
    loop_pos(0, count) =
        coil_radius * cos(overlap_angle + 2 * PI / (float)Ncoils);
    loop_pos(1, count) =
        coil_radius * sin(overlap_angle + 2 * PI / (float)Ncoils);
    loop_pos(2, count) =
        (coil_length - overlap_length) -
        (coil_length - overlap_length) * ((float)pos / (float)RES);
    count++;
  }
  ...
  for (unsigned int pos = 0; pos < loop_pos.n_cols; pos++) {
    loop_current(0, pos) =
        loop_pos(0, (pos + 1) % loop_pos.n_cols) - loop_pos(0, pos);
    loop_current(1, pos) =
        loop_pos(1, (pos + 1) % loop_pos.n_cols) - loop_pos(1, pos);
    loop_current(2, pos) =
        loop_pos(2, (pos + 1) % loop_pos.n_cols) - loop_pos(2, pos);
  }
    ...
    for (int j = 0; j < (int)Ny; j += mapshrink) {
      for (int i = 0; i < (int)Nx; i += mapshrink) {
        ...
        for (unsigned int pos = 0; pos < loop_pos.n_cols; pos++) {
          ...
          fvec diff = S - r;
          float dist =
              (float)pow((float)(diff(0) * diff(0) + diff(1) * diff(1) +
                                 diff(2) * diff(2)),
                         (float)(3.0 / 2.0));
        ...
        for (int kk = sz; kk <= ez; kk++) {
          ...
          for (int jj = sy; jj <= ey; jj++) {
            ...
            for (int ii = sx; ii <= ex; ii++) {
              float wx =
                  wy * (1 - abs((float)ii - (float)i) / (float)mapshrink);
...
void PHANTOM::add_noise(Array<Array<complex<float>, 3>, 2> &kdata) {
  ...
  float mean_kdata = 0;
  float num = 0;
  for (Array<Array<complex<float>, 3>, 2>::iterator miter = kdata.begin();
       miter != kdata.end(); miter++) {
  ...
  for (Array<Array<complex<float>, 3>, 2>::iterator miter = kdata.begin();
    ...
    for (Array<complex<float>, 3>::iterator m2iter = temp.begin();
         m2iter != temp.end(); m2iter++) {
...
Array<int, 3> FRACTAL3D::synthetic_perfusion(int xs, int ys, int zs,
                                             PerfType ptype) {
      ...
      for (int k = 0; k < perf.extent(thirdDim); k++) {
        for (int j = 0; j < perf.extent(secondDim); j++) {
          for (int i = 0; i < perf.extent(firstDim); i++) {
            ...
            float y =
                (j - perf.extent(secondDim) * 0.5) / perf.extent(secondDim);
            float z = (k - perf.extent(thirdDim) * 0.5) / perf.extent(thirdDim);

            if (((x * x) / (0.5 * 0.5) + (y * y) / (0.35 * 0.35) +
                 (z * z) / (0.25 * 0.25)) < 0.75) {
              perf(i, j, k) = 1;
            } else if (((x * x) / (0.5 * 0.5) + (y * y) / (0.35 * 0.35) +
                        (z * z) / (0.25 * 0.25)) < 1) {
              perf(i, j, k) = 4;
            }
          }
...
field<FRACTAL3D::TFRACT_RAND> FRACTAL3D::create_tree(fmat seeds_start,
                                                     fmat seeds_stop, fmat X) {
  // Variables
...
#pragma omp parallel for
    for (int pos = 0; pos < count; pos++) {
      ...
      fvec centroid =
          (XT + wTrunk * tree(pos).start + wBranch * tree(pos).stop);
      ...
      while (to_initial == 0) {
        ...
        if (root == -1) {
          ...
          delta_cost += tree(root).l * (pow(tree(root).Q + 1, qPower) -
                                        pow(tree(root).Q, qPower));
          loc = root;
        }
      }
...
void FRACTAL3D::build_tree(int Nx, int Ny, int Nz) {
  ...
  for (int pos = 0; pos < terminal_pts; pos++) {
    int found = 0;
    while (found == 0) {
      int k = (int)(((float)perf.extent(thirdDim) * (float)rand()) /
                    ((float)RAND_MAX));
      int j = (int)(((float)perf.extent(secondDim) * (float)rand()) /
                    ((float)RAND_MAX));
      int i = (int)(((float)perf.extent(firstDim) * (float)rand()) /
                    ((float)RAND_MAX));
      int val = (int)(((float)perf_max * (float)rand()) / ((float)RAND_MAX));
      if (val < perf(i, j, k)) {
        X(0, pos) = ((float)i + (float)rand() / (float)RAND_MAX - 0.5f -
                     (float)perf.extent(firstDim) * 0.5) /
                    (float)perf.extent(firstDim);
        X(1, pos) = ((float)j + (float)rand() / (float)RAND_MAX - 0.5f -
                     (float)perf.extent(secondDim) * 0.5) /
                    (float)perf.extent(secondDim);
        X(2, pos) = ((float)k + (float)rand() / (float)RAND_MAX - 0.5f -
                     (float)perf.extent(thirdDim) * 0.5) /
                    (float)perf.extent(thirdDim);
        found = 1;
      }
    }
  }
  ...
  // Arterial Tree
  field<FRACTAL3D::TFRACT_RAND> arterial_tree =
      FRACTAL3D::create_tree(seeds_start, seeds_stop, X);
  ...
  // Venous Tree
  field<FRACTAL3D::TFRACT_RAND> venous_tree =
      FRACTAL3D::create_tree(Vseeds_start, Vseeds_stop, X);
  ...
  for (int t = 0; t < (int)arterial_tree.n_elem; t++) {
    ...
    if (n_vessel > 1) {
      for (int ii = 0; ii < n_vessel; ii++) {
        fvec vessel_pos = scale * arterial_tree(t).start +
                          vessel_dir * ((float)ii / (n_vessel - 1));

        float time =
            arterial_tree(t).time + total_time * ((float)ii / (n_vessel - 1));
        ...
        // cout << "Vessel Position = " << xx << "," << yy << "," << zz << " R =
        // " << rr << endl << flush;

        // cout << "Position = " << xx << "," << yy << "," << zz << " R = " <<
        // rr << endl << flush;
        for (int k = -rr; k <= rr; k++) {
          for (int j = -rr; j <= rr; j++) {
            for (int i = -rr; i <= rr; i++) {
              ...
              float s;
              if (R < 1.5) {
                // For small vessels below resolution of matrix need to
                // normalize size
              ...
              if (s > FUZZY((i + xx) % Nx, (j + yy) % Ny, (k + zz) % Nz, 0)) {
                ...
                FUZZYT((i + xx) % Nx, (j + yy) % Ny, (k + zz) % Nz, 0) =
                    s * time;
              }
            }
          }
...
#pragma omp parallel for
  for (int t = 0; t < (int)venous_tree.n_elem; t++) {
    ...
    if (n_vessel > 1) {
      for (int ii = 0; ii < n_vessel; ii++) {
        fvec vessel_pos = scale * venous_tree(t).start +
                          vessel_dir * ((float)ii / (n_vessel - 1));

        float time =
            venous_tree(t).time + total_time * ((float)ii / (n_vessel - 1));
          ...
          for (int j = -rr; j <= rr; j++) {
            for (int i = -rr; i <= rr; i++) {
              ...
              if (s > FUZZY((i + xx) % Nx, (j + yy) % Ny, (k + zz) % Nz, 2)) {
                ...
                FUZZYT((i + xx) % Nx, (j + yy) % Ny, (k + zz) % Nz, 2) =
                    s * time;
              }
            }
          }
...
#pragma omp parallel for
  for (int t = 0; t < (int)tissue_tree.n_elem; t++) {
    ...
    int rr = (int)ceil(R);
    // cout << "Tissue Position = " << xx << "," << yy << "," << zz << " R = "
    // << rr << endl << flush;
  ...
  FUZZYT(Range::all(), Range::all(), Range::all(), 2) =
      1.0 - FUZZYT(Range::all(), Range::all(), Range::all(), 2);
...
inline float gamma_variate(float x, float beta, float alpha) {
  if (x < 0.0) {
    ...
    return ((1.0 / Factorial(alpha) / pow(beta, alpha)) * pow(x, alpha) *
            exp(-x / beta));
  }
}
...
void FRACTAL3D::write_matlab_truth_script(const char *folder) {
  ...
  script << "fseek(fid,(Nx*Ny*Nz + Nx*Ny*(k-1)+Nx*(j-1)+(i-1))*4,'bof'); "
         << endl;
  script << "dens_v = fread(fid,1,'float'); " << endl;
  script << "fseek(fid,(2*Nx*Ny*Nz + Nx*Ny*(k-1)+Nx*(j-1)+(i-1))*4,'bof'); "
         << endl;
  ...
  script << "st = st + "
            "dens_a*dual_gamma_variate((t-toa_a),beta*(1+2*toa_a),alpha);"
         << endl;
  script << "st = st + "
            "dens_v*dual_gamma_variate((t-toa_v),beta*(1+2*toa_v),alpha);"
         << endl;
  script << "st = st + "
            "dens_t*dual_gamma_variate((t-toa_t),beta*(1+2*toa_t),alpha);"
         << endl;

  script << "function s = dual_gamma_variate( x,beta,alpha) " << endl;
  script
      << "s = double(x>0).*(  "
         "(1.0./factorial(alpha)./(beta.^alpha)).*(x.^alpha).*exp(-x./beta));"
      << endl;
  script << "s = s + double(x>0).*(  "
            "(1.0./factorial(alpha)./((4*beta).^alpha)).*(x.^alpha).*exp(-x./"
            "(4*beta)));"
         << endl;
  script.close();
  ...
  script << "s = dens.*dual_gamma_variate((t-toa),beta*(1+2*toa),alpha);"
         << endl;
  ...
  script << "s = s + dens.*dual_gamma_variate((t-toa),beta*(1+2*toa),alpha);"
         << endl;
...
void FRACTAL3D::calc_image(Array<complex<float>, 3> &IMAGE, int t, int frames) {
  ...
  for (int compartment = 0; compartment < FUZZYT.length(fourthDim);
       compartment++) {
#pragma omp parallel for
      ...
      for (int j = 0; j < FUZZYT.length(secondDim); j++) {
        for (int i = 0; i < FUZZYT.length(firstDim); i++) {
          ...
          if (current_time > time) {
            ...
            IMAGE(i, j, k) +=
                dens * gamma_variate(tdiff, beta_t, alpha);  // Bolus
            IMAGE(i, j, k) +=
                0.25 * dens *
                gamma_variate(tdiff, 4 * beta_t, alpha);  // Persistent signal
          }
        }
      }
    }
