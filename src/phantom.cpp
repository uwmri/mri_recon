...
void PHANTOM::init(int Sx, int Sy, int Sz,int St){
    ...
    // Phantom Density
    cout << "Init IMAGE: " << Nx << " by " << Ny << " by " << Nz << " by " << Nt << endl << flush;
...
void PHANTOM::write_matlab_truth_script( const char *folder){
    ...
    switch(phantom_type){
        case(FRACTAL):{
                          cout << "Writing Matlab Scripts" << endl << flush;
...
Array< int,3 > FRACTAL3D::synthetic_perfusion(int xs, int ys, int zs, PerfType ptype){
    ...
    switch(ptype){
        ...
        case(ASL):{
                      ...
                      ArrayRead(perf,"Perfusion_Lung_256.dat");
...
void  FRACTAL3D::build_tree(int Nx, int Ny, int Nz){
    ...
    // Get Seed Points	
    fmat Vseeds_start= zeros<fmat>(3,2);
    fmat Vseeds_stop= zeros<fmat>(3,2);
    ...
    /* Brain
       ...
       */
    // Lungs
    Vseeds_start(0,0) =  0.00;
    Vseeds_start(1,0) =  -0.00;
    Vseeds_start(2,0) =  -0.00;

    Vseeds_start(0,1) =  0.00;
    Vseeds_start(1,1) =  -0.00;
    Vseeds_start(2,1) =  -0.00;

    Vseeds_stop(0,0) =  seeds_stop(0,0);
    Vseeds_stop(1,0) =  seeds_stop(1,0);
    Vseeds_stop(2,0) =  seeds_stop(2,0);

    Vseeds_stop(0,1) =  seeds_stop(0,1);
    Vseeds_stop(1,1) =  seeds_stop(1,1);
    Vseeds_stop(2,1) =  seeds_stop(2,1);
...
#pragma omp parallel for	
    ...
    /*
    Array<float,3>FUZZYref = FUZZY(Range::all(),Range::all(),Range::all(),0);
    ArrayWrite(FUZZYref,"Phantom_A.dat");
    Array<float,3>FUZZYref2 = FUZZY(Range::all(),Range::all(),Range::all(),1);
    ArrayWrite(FUZZYref2,"Phantom_T.dat");
    Array<float,3>FUZZYref3 = FUZZY(Range::all(),Range::all(),Range::all(),2);
    ArrayWrite(FUZZYref3,"Phantom_V.dat");
    */
            ...
            for(int j= 0; j< FUZZYT.length(secondDim); j++){
                for(int i= 0; i< FUZZYT.length(firstDim); i++){
                    ...
                    if( current_time > time){
                        ...
                        IMAGE(i,j,k) += 0.25*dens*gamma_variate(tdiff,4.0*beta_t,alpha); // Persistent signal
                    }
                }
            }
        }
