...
void LOWRANKCOIL::thresh( Array< complex<float>,4 > &image,Array< complex<float>,4 > &temp){
	...
	int shiftx = -(int)floor(block_size_x/2);
	int shifty = -(int)floor(block_size_y/2);
	int shiftz = -(int)floor(block_size_z/2);
	...
	for(int offset=0; offset <2; offset++){
		if(offset ==0){
			shiftx = -(int)floor(block_size_x/2);
			shifty = -(int)floor(block_size_y/2);
			shiftz = -(int)floor(block_size_z/2);
		}else{
			shiftx = 0;
			shifty = 0;
			shiftz = 0;
...
void LOWRANKCOIL::combine( Array< complex<float>,4 > &image, Array< complex<float>,3> &out){
